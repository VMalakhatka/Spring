...
public class MainWindow extends JFrame {
    ...
    public void redrawState(GameState gameState) {
        CellDto[][] field = gameState.field();
        for (int i = 0; i < field.length; i++) {
            for (int j = 0; j < field[i].length; j++) {
                setCellImage(i, j, getCellImage(field[i][j]));
            }
        }
        bombsCounterLabel.setText(String.valueOf(gameState.flagCount()));


    }

    private GameImage getCellImage(CellDto cell) {
        if (!cell.isOpened()) return cell.isBlocked() ? GameImage.MARKED : GameImage.CLOSED;
        return cell.isMine() ? GameImage.BOMB : GameImage.getByNumber(cell.countMineAround());
    }
    ...
    public void setCellImage(int y, int x, GameImage gameImage) {
            ...
            for (int col = 0; col < numberOfCols; col++) {
                ...
                cellButtons[y][x].addMouseListener(new MouseAdapter() {
                    ...
                    public void mouseReleased(MouseEvent e) {
                        ...
                        switch (e.getButton()) {
                            case MouseEvent.BUTTON1 -> listener.onMouseClick(y, x, ButtonType.LEFT_BUTTON);
                            case MouseEvent.BUTTON3 -> listener.onMouseClick(y, x, ButtonType.RIGHT_BUTTON);
                            case MouseEvent.BUTTON2 -> listener.onMouseClick(y, x, ButtonType.MIDDLE_BUTTON);
                        }
                    }
                });
